-- PROJETO_BD_UVV DESDE A CRIAÇÃO DE USÚARIO ATÉ A FINALIZAÇÃO COM INSERÇÃO DE DADOS.

-- CRIANDO O ÚSUARIO:
CREATE ROLE arthur_hardman WITH
  LOGIN
  NOSUPERUSER
  NOINHERIT
  CREATEDB
  NOCREATEROLE
  NOREPLICATION
  ENCRYPTED PASSWORD 'SCRAM-SHA-256$4096:tJrlNk5qg4NYlZDHDdQGGw==$mxeZXvaMIoPIobx+NCx/kPO98GfEJQM/PyWnn/W8q54=:ll/mYVsZSjTMnSA9TVT3GMjtJODnKFHWdthl8A4dybg=';
  
-- CRIANDO BANCO DE DADOS:
CREATE DATABASE uvv
    WITH 
    OWNER = arthur_hardman
    ENCODING = 'UTF8'
    TEMPLATE = template0
...
CREATE SCHEMA IF NOT EXISTS hr
    AUTHORIZATION arthur_hardman;
	
ALTER ROLE arthur_hardman SET search_path TO hr, "$user", public;

-- CRIANDO TABELAS REFERENTES AO PROJETO LÓGICO: si1n_202203760_hr.architect.
...
ALTER TABLE IF EXISTS hr.regioes
    OWNER to arthur_hardman;
	
COMMENT ON TABLE hr.regioes -- Comentário sobre a tabela regioes:
    IS 'Tabela para saber regiao mundial de cada país';
------------------------------------------------------
...
COMMENT ON TABLE hr.paises -- Comentário sobre a tabela paises:
    IS 'Tabela para identificar o nome do país dos endereços das sedes';
-----------------------------------------------------
...
(
    ...
    id_pais character(2) COLLATE pg_catalog."default",
...
COMMENT ON TABLE hr.localizacoes -- Comentário sobre a tabela localizações:
    IS 'Usado para localizar endereços de sedes da empresa';
------------------------------------------------------

CREATE TABLE IF NOT EXISTS hr.cargo
(
    id_cargo_pk character varying(10) COLLATE pg_catalog."default" NOT NULL,
    cargo character varying(35) COLLATE pg_catalog."default" NOT NULL,
    salario_minimo numeric(8,2),
    salario_maximo numeric(8,2),
    CONSTRAINT cargo_pk PRIMARY KEY (id_cargo_pk),
    CONSTRAINT ak_cargo UNIQUE (cargo),
	CONSTRAINT salario_min_check CHECK (salario_minimo >= 0::numeric),
	CONSTRAINT salario_max_check CHECK (salario_maximo >= 0::numeric)
);

ALTER TABLE IF EXISTS hr.cargo
    OWNER to arthur_hardman;
	
COMMENT ON TABLE hr.cargo -- Comentário sobre a tabela cargo:
    IS 'Tabelas que é usada para verificar os cargos e os salários mínimos e máximos entre os cargos';
-------------------------------------------------------
...
(
    ...
    id_cargo character varying(10) COLLATE pg_catalog."default",
    salario numeric(8,2),
	...
	CONSTRAINT salario_check CHECK (salario >= 0::numeric)
);

ALTER TABLE IF EXISTS hr.empregados
    OWNER to arthur_hardman;
	
COMMENT ON TABLE hr.empregados -- Comentário sobre a tabela empregados:
    IS 'Tabela que contem todos os dados do empregado necessários';
COMMENT ON COLUMN hr.empregados.email -- Comentário sobre a coluna email:
    IS 'Preencher somente até o @ do email';
COMMENT ON COLUMN hr.empregados.id_supervisor -- Comentário sobre a coluna id_supervisor:
    IS 'Caso tenha um supervisor entre os empregados para ter conhecimento de quem seria.';
---------------------------------------------------
...
COMMENT ON TABLE hr.departamentos -- Comentário sobre a tabela departamentos:
    IS 'Tabela para especificar o departamento do empregado';
COMMENT ON COLUMN hr.departamentos.id_gerente -- Comentário sobre a coluna id_gerente:
    IS 'Caso tenha um gerente do departamento, essa informação será informada através do relacionamento com a tabela empregados';
------------------------------------------------------
...
COMMENT ON TABLE hr.historico_cargos -- Comentário sobre a tabela histórico de cargos:
    IS 'Tabela para visualizar o histórico de cada empregado com os cargos que passou';
...
-- INSERINDO DADOS NA TABELA LOCALIZAÇÕES:

INSERT INTO hr.localizacoes (id_localizacoes_pk, endereco, cep, cidade, uf, id_pais) VALUES
...
INSERT INTO hr.cargo (id_cargo_pk, cargo, salario_minimo, salario_maximo) VALUES
...
INSERT INTO hr.departamentos (id_departamentos, nome, id_gerente, id_localizacao ) VALUES
...
-- INSERINDO DADOS NA TABELA HISTÓRICO_CARGOS:

INSERT INTO hr.historico_cargos (id_empregado_1, data_inicial, data_final, id_cargo, id_departamentos) VALUES
...
ADD CONSTRAINT localizacoes_departamentos_fk FOREIGN KEY (id_localizacao)
        REFERENCES hr.localizacoes (id_localizacoes_pk) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION;

ALTER TABLE hr.departamentos
ADD CONSTRAINT empregados_departamentos_fk FOREIGN KEY (id_gerente)
        REFERENCES hr.empregados (id_empregado) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION;
...
ADD CONSTRAINT departamentos_empregados_fk FOREIGN KEY (id_departamentos)
        REFERENCES hr.departamentos (id_departamentos) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION;

ALTER TABLE hr.empregados
ADD CONSTRAINT cargo_empregados_fk FOREIGN KEY (id_cargo)
        REFERENCES hr.cargo (id_cargo_pk) MATCH SIMPLE
        ON UPDATE NO ACTION
        ON DELETE NO ACTION;
